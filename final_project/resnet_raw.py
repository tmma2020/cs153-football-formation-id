...
class HudlRawDataset(Dataset):
    def __init__(self, root_dir, metadata_file, transform=None):
        ...
        for entry in metadata:
            ...
            # Match to sideline image
            image_filename = f"sideline_{video_path}.png"
...
def run_resnet_raw():
    ...
    # Load dataset
    dataset = HudlRawDataset(RAW_DATA, DATASET_JSON, transform=transform)
    ...
    print(f"Validation Accuracy on Raw Images: {100 * correct / total:.2f}%")
    ...
    print(f"[INFO] Classification report saved to {os.path.join(output_dir, 'classification_report_raw.txt')}")


    # Save trained model
    model_save_path = os.path.join(output_dir, "resnet_raw_model.pt")
